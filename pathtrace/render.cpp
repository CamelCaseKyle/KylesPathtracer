...
Context::Context(int width, int height, float _fov, char *title) {
	...
	if (found) {
		...
		window = glfwCreateWindow(width * 2, height, title, *monitor, NULL);
	} else {
		//create window with move/resize
		window = glfwCreateWindow(width * 2, height, title, NULL, NULL);
	}
#else
	...
	projection = glm::perspective(fov, aspect, 0.1f, 1000.f);
	//set later
	clearColor = glm::vec4(1.f, 0.f, 0.f, 1.f);
	//all teh matricies
...
bool Context::initPassthrough() {
	...
	passthrough.addUniform("iChannel0");
	...
	uploadBuffer(GL_ELEMENT_ARRAY_BUFFER, &passthrough_inds[0], 6 * sizeof(unsigned int), vbo[v_quad_ind], false);
...
void Context::display(const GLuint target) {
	//render to screen
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	//clear the screen
	...
	glUniform1f(shade.uniform("iGlobalTime"), globalTime);
	//glUniform3f(shade.uniform("loc"), 0.f, 0.f, 0.f);
	//glUniform3f(shade.uniform("vel"), 0.f, 0.f, 0.f);
	//glUniform3f(shade.uniform("iMouse"), 0.f, 0.f, 0.f);
	//glUniform3f(shade.uniform("orient"), 0.f, 0.f, 0.f);
	...
	glDrawElements(GL_TRIANGLES, sizeof(passthrough_inds), GL_UNSIGNED_INT, 0);
