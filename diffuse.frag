...
vec4 renderDiffuse(in vec2 fragCoord, in vec2 iResolution, in int iFrame, in sampler2D gBuffer, in sampler2D lastFrame) {
    bool biased = false;
#ifdef BIASED
    ...
    if (ho != LIGHT) {
        ...
        if (biased) {
            ...
            fcol.rgb += DMIS(hl, hn, ho, seed);
        } else {
            // unbiased sampling (GT)
            fcol.rgb += UnbiasedLambertian(hl, hn, ho, seed);
        }
    }
#ifndef BIASED
    if (biased)
...
void main() {
    fragColor = renderDiffuse(gl_FragCoord.xy, iResolution, iFrame, iChannel0, iChannel1);
}
